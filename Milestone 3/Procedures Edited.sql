...
as
begin
if exists (select* from DeletedGuests where email=@email)
set @success=1;
else
set @success=0;
end;
...
create proc ViewAssignedRoom
@user_id int
AS 
begin
select r.* from Users u inner join Room r on u.room=r.room_id and u.id=@user_id
end;
...
AS
BEGIN
    If not exists (select * from Task where creator = @user_id AND name=@title)
	set @success=-1;
	else
	begin
    UPDATE Task
...
AS
BEGIN
IF NOT EXISTS ( SELECT 1 FROM Device WHERE device_id = @device_id)
 SET @success = 0;
ELSE
 BEGIN
...
BEGIN
	...
	IF @other_user_id  <> -1 AND EXISTS (select 1 from Users where id=@other_user_id)
	begin
	INSERT INTO Calendar (event_id, user_assigned_to, name, description, location, reminder_date)
...
*/
-- insert correctly but does not return -1
...
AS
BEGIN
IF EXISTS (SELECT 1 FROM Calendar WHERE event_id = @event_id) AND EXISTS ( select * from Users where id=@user_id)
	and not exists(SELECT 1 FROM Calendar WHERE user_assigned_to=@user_id AND event_id = @event_id)
		begin
...
ELSE
       select * from Calendar where event_id=198272
...
AND NOT EXISTS (select * from RoomSchedule where creator_id=@creator_id AND start_time=@start_time)
begin
INSERT INTO RoomSchedule(creator_id, action, room, start_time, end_time)
    VALUES (@creator_id, @action, @room_id, @start_time, @end_time);
	set @success=1
	end
ELSE IF EXISTS (select * from RoomSchedule where creator_id=@creator_id AND start_time=@start_time)
set @success=-1
ELSE
set @success=0
END;
...
BEGIN
	...
	select @email=email from Users where id=@guest_id

	INSERT INTO DeletedGuests VALUES(@email);
	DELETE FROM Communication where reciever_id=@guest_id
	DELETE FROM RoomSchedule where creator_id=@guest_id
	DELETE FROM Log where user_id=@guest_id
	DELETE FROM Recommendation where user_id=@guest_id 
	DELETE FROM Camera where monitor_id=@guest_id
...
BEGIN
  ...
  set @success=5
  ELSE IF not exists (select* from Room where room_id=@room_id)
  set @success=3;
  ELSE
  BEGIN
...
AS
BEGIN
IF NOT EXISTS (SELECT * FROM Room where room_id=@location) 
SET @success = 0
else if EXISTS (SELECT * FROM Device where device_id=@device_id)
set @success=-1
else
begin
...
AS
BEGIN
IF NOT EXISTS (SELECT 1 FROM Device WHERE battery_status = 0)
SET @success = 0
ELSE
begin
	UPDATE Device
